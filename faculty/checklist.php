		...
		<!--<form  method = "POST" class="form-inline" action="sort_checklist.php">
			...
			<div class="controls">
				...
				<select name="instructor" id="instructor" required>
					<option value=""></option>
					<?php 
						$faculty_query = mysql_query("select * from tbl_faculty") or die(mysql_error());
						while($fac = mysql_fetch_array($faculty_query)){
							 echo '<option value="'.$fac['emp_no'].'">'.$fac['fname'].' '.$fac['mname'].' '.$fac['lname'].' </option>';
						} 
					?>
				</select>		
